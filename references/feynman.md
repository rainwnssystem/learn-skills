...
The dispatcher (`SKILL.md` at the skill root) has already parsed `<concept>` and `<lang>`. Proceed through the steps **in order**.

## Output language (`<lang>`)

`<lang>` defaults to English. When non-English, the Step 1 prompt to the user, the Step 2 diagnosis table and comments, the Step 3 follow-up question, the Step 4 corrections, recall questions, and the recommended-revisit line are all written in that language.

Concept names, code identifiers, command flags, protocol / RFC names stay verbatim even inside translated prose. If `<lang>` was ambiguous or unrecognized, the dispatcher already fell back to English and noted the fallback in a one-line prefix before Step 1.
...
5. End with a **recommended next revisit time** (e.g., "Recommend `/learn {concept} --feynman` again in 3 days").

## Principles

- Remember this is the **post-learning** stage. If the concept is new, suggest `/learn` first (translate the quoted line into the `<lang>` language):
  > "This concept seems new to you. It is more efficient to learn it first via `/learn {concept}`, then return with `/learn {concept} --feynman`. Proceed anyway?"
